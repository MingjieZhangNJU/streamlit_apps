...
st.title('UTCI Calculator from NJU_GUBS')
st.subheader("GUI小悦帮着优化下")
st.write("")
st.write("")
par1=st.slider("空气温度Ta℃",-10.0,50.0)
par2=st.slider("相对湿度RH%",0.0,100.0)
par3=st.slider("平均辐射温度Tmrt℃",0.0,80.0)
par4=st.slider("风速值V m/s",0.0,10.0)
st.write("")
st.write("")
res=st.markdown("UTCI计算结果")
if st.button("计算"):
    UTCI=utci_calculator(par1, par2, par3, par4)
    st.subheader(UTCI)
